...
async function requestJson(path,options={},code='request_failed'){
  if(localOnly && options.method && options.method !== 'GET')throw new Error('local_only_write_blocked');
...
try{
  ...
  if(localOnly){
    save('tock-dashboard-staged.json',dashboard);
    console.log(JSON.stringify({published:false,staged:true,verified:true,checkedAt:dashboard.checkedAt,exportCheckedAt:dashboard.exportCheckedAt,totals:dashboard.totals}));
  }else{
