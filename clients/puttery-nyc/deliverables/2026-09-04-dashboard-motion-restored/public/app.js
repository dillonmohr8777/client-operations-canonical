const platformDetails = [
  ...
  },
  {
    ...
    confirmed: "NYC business 37824 is connected to reservation exports and webhook updates. The dated source panel provides the current ingestion evidence. Delivery events include updates, not separate bookings.",
    unknown: "Controlled tagged-booking confirmation, delivery counter reconciliation, GA4 settings, consent and campaign-field preservation.",
    fallback: "Retain the last complete export snapshot if a refresh fails; automatic processing requires the Windows host online.",
  },
  {
    key: "Toast",
    name: "Toast",
    state: "ready",
    stateLabel: "Read-only reporting verified",
    role: "Supplies aggregate NYC orders, payments, net sales, refunds, and voids for operational reconciliation.",
    confirmed: "Read-only reporting is active for Puttery - New York City, Restaurant ID 154035. The dashboard carries a dated closed-day aggregate with no guest or order identifiers.",
    unknown: "Approved finance authority, closeout rule, API or export route, revenue-center inclusion map, and transaction-level Tock matching remain unverified.",
    fallback: "Refresh the closed-day Sales Summary through the verified read-only account and keep Toast totals separate from attribution.",
  },
  {
    key: "Tripleseat",
    name: "Tripleseat",
  ...
  },
  {
    ...
    confirmed: "Tock confirmed that walk-ins omit reservationId and use walkinId. The NYC normalizer accepts that identifier, and the latest complete export has zero excluded rows.",
    unknown: "The approved value and attendance treatment for walk-ins, and reconciliation to any realized-sales source, remain unverified.",
    fallback: "Include normalized walk-ins in reservation-state counts, but do not claim walk-in revenue or completed visits until the approved definitions and source reconciliation are complete.",
  },
  {
    key: "GTM",
...
const questions = [
  ...
  { id: "tock-04", platform: "Tock", blocker: true, owner: "Momentum", prompt: "Do reservation webhooks include stable reservation ID, business ID, status, sequence ID, value, and refund details for this account?", why: "The live payload, not the public model, determines the usable schema.", evidence: "Redacted create, edit, cancel, no show, and refund payload samples.", vendorAnswer: { text: "Live Puttery NYC reservation webhooks and exports are being normalized and merged by source identifier and version. Representative state and financial-field presence is verified; controlled coverage of every cancellation, no-show and refund variant is still required before revenue reporting.", source: "Live Puttery NYC source verification, 2026-09-08" } },
  ...
  { id: "toast-01", platform: "Toast", blocker: true, owner: "Puttery", prompt: "What is the Puttery NYC restaurant GUID, management group, and export restaurant number?", why: "Toast standard requests are location specific and names are not safe join keys.", evidence: "Restaurant GUID and location folder mapping.", vendorAnswer: { text: "Read-only Toast reporting is active for Puttery - New York City. The verified Toast Restaurant ID is 154035; API GUID and export-folder mapping remain separate open fields.", source: "Toast Web read-only account, 2026-09-19" } },
  ...
  { id: "toast-03", platform: "Toast", blocker: true, owner: "Puttery", prompt: "Who has Manage Integrations, Sales Reports, Data Export Config, and required multi location permissions?", why: "Incomplete roles can expose only part of the location or disable required routes.", evidence: "Named users and permission checklist.", vendorAnswer: { text: "Sales Summary reporting is readable for the NYC location. Integration, export-configuration, and multi-location administration scopes were not changed or inferred from report access.", source: "Toast Web read-only verification, 2026-09-19" } },
  ...
  { id: "walkin-01", platform: "Tock Walk-in", blocker: false, owner: "Puttery", prompt: "Does Puttery NYC record walk-in guests in Tock, and with what value?", why: "The kickoff assumed walk-ins were only reachable through Toast; if Tock records them, a deferred source becomes unnecessary.", evidence: "A representative walk-in record with its value fields.", vendorAnswer: { text: "Laura confirmed that Tock walk-ins do not include reservation IDs and do include walk-in IDs. The normalizer now accepts walkinId, and the latest complete export has zero excluded rows. Approved value treatment remains a separate finance decision.", source: "Laura Benedetto, Technical Account Analyst, Resy / Tock / American Express Global Dining, 2026-09-08" } },
...
function renderPlatforms() {
  ...
  const tock = platformDetails.find(platform => platform.key === 'Tock');
  if (operational?.tockConfirmed) Object.assign(tock, {
    state: 'ready', stateLabel: operational.webhookState || 'Dated snapshot',
    confirmed: operational.tockConfirmed, unknown: operational.tockPending || tock.unknown,
    fallback: 'The last complete export snapshot is retained when a refresh fails. Automatic processing currently requires the Windows host online.'
  });
  const container = document.querySelector("#platform-grid");
  const activePlatform = container.contains(document.activeElement) ? document.activeElement.dataset.platformJump : null;
    ...
    return `
      <article class="platform-card">
        <div class="platform-name">
          <h3>${platform.name}</h3>
          <span class="platform-state ${platform.state}">${escapeText(platform.stateLabel)}</span>
  ...
  container.querySelectorAll("[data-platform-jump]").forEach((button) => {
    if (activePlatform === button.dataset.platformJump) button.focus({ preventScroll: true });
    button.addEventListener("click", () => {
      document.querySelector("#platform-filter").value = button.dataset.platformJump;
      renderQuestions();
      document.querySelector("#implementation-notes").open = true;
    ...
    return `
      <details class="question-group" ${groupIndex === 0 ? "open" : ""}>
        <summary>
          ...
          <span class="question-group-progress">${resolved} answers recorded</span>
...
function exportAnswers() {
  ...
  const payload = {
    ...
    evidenceBoundary: "Discovery answers are user-entered browser notes, not verified access. Tock operating aggregates are reported separately with their source timestamps; campaign attribution and revenue remain pending validation.",
    connectedBusinessSystems: window.PUTTERY_DASHBOARD?.data ? ["Tock"] : [],
    aggregateSnapshot: window.PUTTERY_DASHBOARD?.data ? { checkedAt: window.PUTTERY_DASHBOARD.data.checkedAt, source: window.PUTTERY_DASHBOARD.source, refreshState: window.PUTTERY_DASHBOARD.refreshState } : null,
...
function setMetricView(period) {
  if (window.PUTTERY_DASHBOARD) window.PUTTERY_DASHBOARD.render(period);
}
