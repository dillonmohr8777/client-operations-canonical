...
Six verified reservation-source counts sit in one square, 2px black frame. Each cell uses black rules, Fugaz One integer values, muted labels, and a precise source or field-presence line. It becomes 3 columns at 1220px and 2 columns below 720px. Never turn these cells into independent floating cards. The adjacent trend reuses teal, square bars, a written scale, keyboard focus and an exact-count table; dense periods scroll inside the chart.
...
The hot-pink strip directly below the header states that Tock operations are connected while attribution, attendance and financial reporting remain pending. It becomes sticky at 980px and remains text-complete on mobile. This disclosure is part of the product's trust model, not a dismissible notice.

### Champion Putt Intro

Dillon explicitly requested a champion golfer putting into a Puttery reveal. The generated film is a 1280 by 720, 6.041667-second silent clip. The exact locked SVG is a separate HTML overlay; generated video never supplies lettering. The sweep and mask run from 2.7 to 4.0 seconds on the actual video clock, then settle for 650ms after the film ends. The desktop SVG lands in the black space beside the golfer. Portrait mobile keeps the entire 16:9 film visible with `object-fit: contain` and puts the mark above its frame, leaving 28px between mark and film at the inspected 390 by 844 viewport.

Skip, Escape, Pause/Resume and Replay remain available. Controls retain the original pill, black/white/pink palette and yellow focus ring, with a minimum 44px touch height. Intro controls use 0.875rem text on desktop and 0.75rem on mobile. A versioned intro-seen key is separate from discovery answers. Failed media runs the original 2.1-second staged intro. Reduced motion skips video and preserves the static page wordmark. Hidden pages pause playback.

The narrow side golf film replaces only the added fallback slot when ready; the original course ornament, all 14 original keyframes, 6-second header-logo loop and animated section icons remain. The film pauses offscreen, with global Pause, on hidden pages, and under reduced motion. A late media error restores the CSS golf fallback.
...
<!-- September 8 reporting-first cleanup: preserve incumbent tokens and motion. The hero presents reporting stage, connected source, next milestone and revenue validation state. Remove question-completion score treatment. Implementation notes use a native closed details disclosure with worksheet-local controls; technical tables remain progressively disclosed. -->
