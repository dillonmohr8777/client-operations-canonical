# Top 30 Story Shortlist, branded edition (2026-08-10)

Branded restyle of the 10 Aug 2026 Top 30 story shortlist. Four deliverable
files, same data, one design system:

- `Align_HCM_Top30_Story_Shortlist_Branded.xlsx` — the working file. Navy
  masthead with the exact Align logo, brand palette throughout, editable
  yellow columns preserved (Reference Status / Validator / Validation Notes),
  live TOTAL formulas, freeze panes and autofilter, orange tab.
- `Align_HCM_Top30_Story_Shortlist_Branded.html` — the twin read view. Same
  four sections (shortlist, reference candidates, representation gaps, method
  notes), client logo wall, search + platform filters + sort, stat band,
  print-ready CSS.
- `Align_HCM_Top30_Story_Shortlist_Editorial.pdf` — 20-page editorial print
  edition, generated downstream by the Align HCM branded deliverable workflow
  (ReportLab, 10 Aug 2026 19:57 UTC) from this folder's data and provenance
  notes. Cover with stat band, executive readout, client identity index,
  ranked story cards (4 per page), reference candidates, both gap parts, and
  the method notes verbatim. Verified against the source workbook: all 30
  ranks and true-USD values match exactly, counters match ($8.26M, 20/30,
  16/30, 14 industries). Real official client marks are embedded per card;
  most are white-ink variants baked onto brand-navy `#0A1628` tiles (seamless
  on the navy cards, but not reusable as transparent assets), and the Align
  logo is the exact extracted mark on every page. Known gaps if regenerated:
  body type is DejaVu Sans / Helvetica instead of brand fonts, and a few card
  strings truncate with an ellipsis.
- `Align_HCM_Top30_Story_Shortlist_Living.html` — interactive living variant
  of the shortlist, generated by `build_living_html.py` from the same data.
- `Align_HCM_Top30_Command_Deck.html` — the interactive dark dashboard. Six
  views on the master dark template (navy `#071120` / `#0A1628`, glass panels,
  glow blobs, exact logo in the header, constellation core, and footer): a
  Command Deck overview with an orbiting client constellation (three rings by
  readiness, node size by story value), the Stories card grid with a detail
  drawer, a live Pipeline view, Validation Ops owner boards with copyable ask
  lists, a Coverage radar, and the Reference Bench. Deep HubSpot integration,
  all read from portal 242825734 on 10 Aug 2026: per-client deal rollups
  matched by domain (28 of 30), won vs open splits, a 2019 to 2026 engagement
  heat matrix, portal-wide pipeline pulse, owner-roster status (which flags 12
  stories owned by an inactive user), and per-client deep links into HubSpot
  company records. Chart palette (`#F05A28 #4E86C8 #1FA98C #BA841A`) validated
  for the dark surface with the dataviz six-check validator. Validation
  checklists and notes persist in localStorage; current filter exports to CSV.
  Regenerates via `build_command_deck.py`.

## What was added vs the source file

Story rows, values, and the three TOTAL formulas are unchanged from the
source build. Three columns were appended on the Top 30 tab, read from
HubSpot portal 242825734 on 10 Aug 2026 (read-only): **Website**, **HQ (CRM)**,
**Employees (CRM)**. In the HTML twin the same values fold into the client cell.

CRM domain divergences, resolved to public primary domains for display and
logos: OhioHealth (CRM record carries `ohiohealthems.com`), MTA NY (CRM
carries `mtany.org`), McCain Foods (CRM carries `mccain.ca`). JACAM Catalyst
and REI had no usable CRM domain; public domains used. Ace Parking's CRM
record is a project entry ("UKG Pro WFM Flip"), so its HQ stays blank.

## Brand provenance

Tokens and the logo come from the shipped Align editorial PDF set (Aug 2026):
navy `#0A1628`, orange `#F05A28` / `#FF6B2B`, rust `#AD3D1B`, teal `#136E61`,
paper `#FCFAF7`, panel `#F5F1EA`, hairline `#E9E4DC`. The logo in
`assets/align-hcm-logo-reverse.png` is the exact production mark extracted
from those PDFs with its transparency mask (268x108), not a recreation.
Canonical brand page: `dillon-os/02_FullTimeJob/AlignHCM/brand-guidelines.md`.

## Client logos

The HTML loads each client mark from its public domain via
`logo.clearbit.com/<domain>` at view time (transparent marks on a white chip)
and falls back to a styled monogram offline. No third-party logo files are
redistributed in this repo.

## Regenerate

`python3 build_deliverables.py` (needs `openpyxl`, `pillow`; reads the source
xlsx path at the top of the script). The build injects cached values for the
three formulas and sets `fullCalcOnLoad` because the remote sandbox has no
LibreOffice Calc; Excel re-verifies the formulas on open.

The editorial PDF and living HTML regenerate via `fetch_client_logos.py`,
`build_align_top30_pdf.py`, and `build_living_html.py`; the GitHub Action
`.github/workflows/align-top30-logo-pdf.yml` runs that pipeline where client
logo domains are reachable.
