...
- The live clip is **rate-locked** to its window (`clipSeconds / filmSeconds`,
  ...
  never by seeking — seeking a playing 1080p element stalls it. Measured drift
  during playback stays under about 0.3s.
- All seeks run through one serial queue, and while paused the render loop keeps
  driving that queue until every visible sheet has an image.
...
and as the source of the freeze frames. Nothing was re-encoded, re-timed at the
file level, or regenerated. Momo is only ever your footage.
