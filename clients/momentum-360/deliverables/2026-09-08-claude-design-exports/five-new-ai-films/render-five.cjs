...
const films = fs.readdirSync(path.join(root,'source')).filter(f => /^AI 0[1-5] - .*\.html$/.test(f)).sort();
const hash = b => crypto.createHash('sha256').update(b).digest('hex');
const log = o => process.stdout.write(JSON.stringify(o)+'\n');
(async()=>{
 const browser = await chromium.launch({channel:'chrome',headless:true});
 const receipt = {started:new Date().toISOString(),full,sourceArchiveSHA256:'05EFBD5710AA91EFFF3709F25C4907A53D248ED336D3A9387E7DF258201FDB6C',films:[]};
 try {
  for(const filename of films){
   ...
   await page.goto('http://127.0.0.1:58000/'+encodeURIComponent(filename),{waitUntil:'networkidle',timeout:60000});
