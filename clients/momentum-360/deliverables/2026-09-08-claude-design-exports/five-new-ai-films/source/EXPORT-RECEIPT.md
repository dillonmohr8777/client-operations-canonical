...
| `film-kit.js` | Shared runtime for the five AI films: alpha-preserving renderer, deterministic 30 fps clock, player + keyboard + drag, authored 16:9 / 9:16 switch, DOM-only preview background, in-scene brand end card, per-particle sized points, per-frame tube rebuilds, original sound synthesis. |
| `AI 01 - Atomic Assembly.dc.html` | 18 s. One lit atom, one impulse, an ordered chain reaction through 3 000 particles on sixteen forking branches, three coordinated structures (helix / lattice / torus band) joining into one sculptural system with a contour pulse, then the exact logo. "One idea. A chain reaction." |
| `AI 02 - Clear the Noise.dc.html` | 18 s. A dense field of 460 original marks at every depth; an oblique tidal ribbon passes through and every mark it reaches inherits the ribbon’s own displacement and is carried out of frame; one signal travels, a lens bends it, it resolves. "Make room for Momentum." |
| `AI 03 - The Hours Return.dc.html` | 18 s. A suspended kinetic dial in macro — instanced bevelled ticks, beads on rings — where six tangled pathways coordinate at their own depths through causal handoffs, then the mechanism opens into negative space. "Your day has bigger plans." |
| `AI 04 - Break the Frame.dc.html` | 18 s. WHAT IF as genuine dimensional letterforms (slice-stack solids), contracting to MAKE IT WHAT IS with changing inserts, fragmenting on a grid sampled from the real glyph coverage, opening a prismatic aperture the camera flies through, and returning as WHAT IS solid from a new angle. "Give your impossible some Momentum." |
| `AI 05 - System Awake.dc.html` | 18 s. An asymmetric machined object, dormant but lit; a pulse rides its actual contour and each shell opens only after the pulse reaches it; the energy ribbon escapes, sketches a new spatial form and settles. "This is what next feels like." |
| `QA-alpha-check.html` | Alpha + timing harness: drives all five deterministic clocks together and composites the SAME captured RGBA layer over white and over navy, then reads those exact pixels back. |
| `REFERENCES.md` | Reference → borrowed technique → original Momentum transformation → film, with the access limitation stated. |
| `FILM-MANIFEST.md` | Five-film manifest: durations, aspects, beat map, render hook, alpha PNG path, produced-vs-pending. |
...
## Five AI launch films — build and test pass

Campaign **Intelligence made visible**: five separate films, five materially different shot structures, one shared player. No Momo, mascot, ceramic character, book, page turn, desk or reused composition appears in any of them. Bricolage Grotesque is the approved display exception; Nunito Sans carries all reading type; the wordmark is the exact master, drawn as an in-scene textured plane so one RGBA pass carries object *and* brand finish.

**Transparency.** Every film renders with `alpha: true`, clear alpha 0, no scene background, no fog, no ground plane, no shadow catcher and no vignette plate. Particle and mark blending is normal rather than additive, so destination alpha accumulates instead of leaving holes; the one glow helper that needs additive colour also adds source alpha. The preview background (navy / white / checker) is a DOM layer *behind* the canvas and is never captured. Verified by reading real pixels at 0.4 / 1.4 / 5.5 / 6.0 / 9.4 / 11.0 / 13.0 / 16.4 / 17.9 s in both aspects: **all five pass — 0 dirty pixels in fully-clear areas, real soft antialiased edges present, no baked-background signature, corners clear wherever the subject is not deliberately touching the frame edge.**

**Determinism.** `renderFrameAt(n)` is a pure function of the authored frame — no accumulated state, no wall clock, all noise index-hashed — so frame *n* is identical on every pass, in either aspect. 540 frames per aspect per film; hook and PNG-sequence loop documented in `FILM-MANIFEST.md`.

### Defects found and repaired

- **Portrait never re-sized the renderer.** The stage went to 9:16 but the canvas stayed at the landscape size, so portrait would have exported letterboxed. The runtime now catches any size mismatch on the next rendered frame and resizes synchronously on the aspect switch, instead of trusting the ResizeObserver alone.
- **AI 02 dimmed distant marks by darkening their colour**, which made them vanish against navy and read as holes in the RGBA layer. Depth is now carried by scale and a narrow brightness band; removal is scale-to-zero.
- **AI 04's fly-through passed through an empty frame.** The aperture and the returned type sat behind the lens mid-move. Both were repositioned ahead of the camera (aperture at z‑=‑1.6, returned type at z‑=‑5.4), the type now arrives *during* the pass, and the gold key light that was turning the letter slices brown was cut back.
- **AI 01 read as random dust**, which the brief explicitly ruled out. Particles now propagate along sixteen forking branches, the count rose to 3 000, the three structures were tightened and the camera stops at the formation instead of flying through it. A gold tube drawn over the merged system read as a separate hoop and was removed — the contour pulse is carried by the particles themselves.
- **AI 01's opening was cropped** and its gold orbit sat edge-on as a straight streak; the camera pulled back and no ring is now edge-on to the opening move.
- **AI 05's gold shell opened so far it detached** from the object; hinge travel and offset were reduced and the framing widened.
- AI 02's ribbon was a near-opaque sheet filling frame (now a light crest at 0.17 opacity on a narrower surface), and its signal took too long to appear (travel accelerated, resolve moved earlier).

No console output on load or across any seek in either aspect after the repairs.

### Produced vs pending — honestly

Produced: five runnable film pages, the shared runtime, the QA harness, the manifest and references, and a working per-frame RGBA capture path callable now. **Not produced and not claimed:** no PNG sequences were written to disk here (nobody has run 5 400 captures), and **no MP4, ProRes or transparent master file exists** — an HTML preview is not a rendered video, and any review MP4 must be labelled opaque rather than called a transparent master.

Nothing purchased, no subscription, no account, no MCP, no new dependency, no upload, publish, deploy or external send. The paused `Flagship - Built Not Prompted.dc.html` and all earlier films are untouched.
