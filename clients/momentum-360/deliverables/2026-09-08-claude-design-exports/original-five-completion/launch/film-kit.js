...
export function buildEndCard(T, logoImg, opts) {
  const o = opts || {};
  const g = new T.Group();
  const wide = o.portrait !== true;
  const W = wide ? 5.4 : 3.9;

  const logoTex = imageTexture(T, logoImg);
  const lw = wide ? 2.5 : 2.35;
  const lh = lw * (logoImg.height / logoImg.width);
  const logo = new T.Mesh(new T.PlaneGeometry(lw, lh), new T.MeshBasicMaterial({
    map: logoTex, transparent: true, depthWrite: false, side: T.DoubleSide, toneMapped: false
  }));
  logo.position.set(0, wide ? 0.66 : 1.16, 0);
  g.add(logo);

  const kick = new T.Mesh(new T.PlaneGeometry(W, W * 0.09), new T.MeshBasicMaterial({
    map: textTexture(T, (x, w, h) => {
      x.font = '900 ' + Math.round(h * 0.4) + 'px Nunito, sans-serif';
      x.letterSpacing = Math.round(h * 0.075) + 'px';
      x.fillStyle = CSS.gold;
      x.textAlign = 'center'; x.textBaseline = 'middle';
      x.fillText((o.kicker || 'INTRODUCING OUR AI DIVISION').toUpperCase(), w / 2, h / 2);
    }, 1600, Math.round(1600 * 0.09)),
    transparent: true, depthWrite: false, side: T.DoubleSide, toneMapped: false
  }));
  kick.position.set(0, wide ? -0.12 : 0.38, 0);
  g.add(kick);

  const face = o.face || 'Archivo, sans-serif';
  const headH = wide ? 0.62 : 0.9;
  const head = new T.Mesh(new T.PlaneGeometry(W, headH), new T.MeshBasicMaterial({
    map: textTexture(T, (x, w, h) => {
      const lines = String(o.line || '').split('|');
      const fs = Math.round(h / lines.length * 0.66);
      x.font = '800 ' + fs + 'px ' + face;
      x.textAlign = 'center'; x.textBaseline = 'middle';
      const lh2 = h / lines.length;
      lines.forEach((ln, i) => {
        const y = lh2 * (i + 0.5);
        const t = ln.trim();
        const dot = t.endsWith('.');
        const body = dot ? t.slice(0, -1) : t;
        const bw = x.measureText(body).width;
        const dw = dot ? x.measureText('.').width : 0;
        const x0 = w / 2 - (bw + dw) / 2;
        x.fillStyle = o.dark ? CSS.navy : CSS.white;
        x.textAlign = 'left';
        x.fillText(body, x0, y);
        if (dot) { x.fillStyle = CSS.gold; x.fillText('.', x0 + bw, y); }
      });
    }, 1600, Math.round(1600 * headH / W)),
    transparent: true, depthWrite: false, side: T.DoubleSide, toneMapped: false
  }));
  head.position.set(0, wide ? -0.74 : -0.46, 0);
  g.add(head);

  g.userData = { logo: logo, kick: kick, head: head, parts: [logo, kick, head] };
  g.visible = false;
  return g;
}
...
export function playEndCard(card, k) {
  const p = card.userData.parts;
