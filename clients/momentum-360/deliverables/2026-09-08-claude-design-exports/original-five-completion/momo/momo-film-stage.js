...
(function () {
  ...
  /* A designed stand-in for a video sheet, in Momentum colours, used whenever a
     clip has not yielded a decodable frame yet. It is brand material, never a
     stand-in for footage that is claimed to be footage. */
  function drawSlate(title) {
    const W = 1280, H = 720;
    const cv = document.createElement('canvas');
    cv.width = W; cv.height = H;
    const g = cv.getContext('2d');
    g.fillStyle = C.paper; g.fillRect(0, 0, W, H);
    g.fillStyle = C.pale; g.fillRect(0, 0, W, 8);
    const pad = 88;

    g.fillStyle = C.muted;
    g.font = '800 24px Nunito, sans-serif';
    let mx = pad;
    for (const ch of 'IDEAS INTO MOTION') { g.fillText(ch, mx, pad + 26); mx += g.measureText(ch).width + 2.1; }

    g.font = '900 84px Archivo, sans-serif';
    g.fillStyle = C.navy;
    ...
    for (const w of words) {
      ...
      if (g.measureText(probe).width > W - pad * 2 && cur) { lines.push(cur); cur = w; } else { cur = probe; }
    }
    if (cur) lines.push(cur);
    let ly = H / 2 - (lines.length - 1) * 46 + 10;
    for (const line of lines) { g.fillText(line, pad, ly); ly += 92; }
    const lastW = g.measureText(lines[lines.length - 1]).width;
    g.fillStyle = C.gold;
    g.fillText('.', pad + lastW, ly - 92);

    g.fillStyle = C.line; g.fillRect(pad, H - pad - 34, W - pad * 2, 1);
    g.fillStyle = C.muted;
    g.font = '800 22px Nunito, sans-serif';
    mx = pad;
    for (const ch of 'MOMENTUM DIGITAL · AI DIVISION') { g.fillText(ch, mx, H - pad); mx += g.measureText(ch).width + 2.1; }
    return cv;
  }
  ...
  class MomoFilmStage extends HTMLElement {
    ...
    async buildSheets() {
      ...
      const clipDefs = [
        { key: 'reveal', src: d('srcReveal'), still: d('stillReveal'), clipStart: 0.15, slate: 'Momo Reveal', w: 4.6, h: 2.59, seg: [72, 44] },
        { key: 'money', src: d('srcMoney'), still: d('stillMoney'), clipStart: 5.5, slate: 'Momo Watches the Money', w: 4.6, h: 2.59, seg: [72, 44] },
        { key: 'great', src: d('srcGreat'), still: d('stillGreat'), clipStart: 7.0, slate: 'Great Ones Build Momentum', w: 4.6, h: 2.59, seg: [72, 44] }
      ];
      ...
      for (const def of clipDefs) {
        ...
        /* inert: a texture source, not a player — no host media chrome, focus,
           PiP or remote-playback surface may attach to it */
        ...
        /* The BASE texture is a baked still of this clip's own footage, taken
           from the supplied master. Image decode never fails, so Momo and both
           films are on the sheets from the first frame — live video enhances the
           sheet, it does not gate whether there is anything to see. The drawn
           slate is a last resort for a missing file. */
        const stillImg = await loadImage(def.still);
        let baseTex;
        if (stillImg) {
          baseTex = new THREE.Texture(stillImg);
          baseTex.needsUpdate = true;
        } else {
          baseTex = new THREE.CanvasTexture(drawSlate(def.slate));
        }
        ...
        const rec = { key: def.key, el, tex, baseTex, hasStill: !!stillImg, clipStart: def.clipStart, url: src, attached: false, failed: false };
        this.videos.push(rec);
        this.sheets.push(this.makeSheet(def.key, baseTex, def));
        /* a decode failure resolves to the baked still, once, and never retries:
           an unhandled error state is what surfaces a native media-error overlay
           and leaves a sheet waiting on a frame that never arrives */
        el.addEventListener('error', () => {
          const er = el.error;
          rec.failed = true;
          const sh = this.sheets.find(x => x.key === rec.key);
          if (sh) sh.mat.uniforms.uMap.value = rec.baseTex;
          console.warn('[momo-film-stage] decode failed for ' + rec.key + '; holding the baked still', er && er.code, er && er.message);
          this.dispatchEvent(new CustomEvent('momo-state', {
            bubbles: true, detail: { playing: this.playing, t: this.t, mediaFallback: rec.key }
          }));
        });
      }
    ...
    loop() {
      ...
      if (this.playing) {
        ...
        const waiting = this.videos.some(v => {
          ...
          return v.key === live && v.el.readyState < 2 && v.seekPending;
        });
    ...
    applyFrame(t, hardSync) {
      ...
      for (const s of this.sheets) {
        ...
        s.mat.uniforms.uWave.value = k.wave;
      ...
      this.postMat.uniforms.uAmp.value = rip.s;
    ...
    /* Acquire the single decoder, park on the featured still, keep it, release.
       Runs through the same serial queue, so it can never collide with playback
       or with another clip's capture. This is what lets a sheet that is NOT live
       obtain its freeze frame — without it a non-live clip is never attached and
       could never capture anything. */
    enqueueFreeze(v) {
      if (v.freezeTex || v.freezePending) return;
      v.freezePending = true;
      this._q = (this._q || Promise.resolve()).then(async () => {
        try {
          if (v.freezeTex) return;
          /* never steal the decoder from the clip that is currently moving */
          const live = this.liveKey(this.t);
          if (this.playing && live && live !== v.key) return;
          const win = WINDOWS[v.key];
          const still = v.clipStart + win[2];
          this.ensureSrc(v);
          if (v.el.readyState < 1) await this.waitFor(v.el, 'loadedmetadata', 4000);
          if (v.el.readyState >= 1) {
            try { v.el.currentTime = still; } catch (e) {}
            if (v.el.seeking || v.el.readyState < 2) await this.waitFor(v.el, 'seeked', 4000);
          }
          if (!this.captureFreeze(v)) {
            await this.waitFor(v.el, 'loadeddata', 1500);
            this.captureFreeze(v);
          }
          if (this.liveKey(this.t) !== v.key) this.releaseSrc(v);
        } finally {
          v.freezePending = false;
        }
      });
    }
    ...
    /* Try to give each clip a still of its own featured frame, one decoder at a
       time, under a hard overall deadline. Whatever fails simply keeps its
       slate — the film opens either way. */
    async prepareFreezes() {
      const deadline = Date.now() + 9000;
      for (const v of this.videos) {
        if (Date.now() > deadline) break;
        this.enqueueFreeze(v);
        await Promise.race([
          (this._q || Promise.resolve()),
          new Promise(r => setTimeout(r, Math.max(500, deadline - Date.now())))
        ]);
      }
    }
    ...
    syncVideos(t, hard) {
      ...
      for (const v of this.videos) {
        ...
        if (!visible) {
          ...
          if (v.key !== live && !v.freezePending && v.freezeTex) this.releaseSrc(v);
          v.armed = false;
          continue;
        }
        ...
        if (v.key !== live || v.failed) {
          v.armed = false;
          if (!v.freezeTex && v.attached && !v.failed) {
            /* free continuity: its window just ended and it still holds a
               decoded frame, so keep that exact frame instead of snapping back
               to the baked still. Purely opportunistic — the baked still already
               guarantees there is something to see. */
            this.captureFreeze(v);
          }
          /* hold the decoder until the still is safely captured */
          if (v.attached && v.freezeTex && !v.freezePending) this.releaseSrc(v);
          this.setMap(sheet, v.freezeTex || v.baseTex);
          continue;
        }

        this.ensureSrc(v);
        /* Attaching a src does not mean there is a picture yet — a 1080p clip can
           take several seconds to yield its first frame. Show the last good image
           (its still, else its slate) until the decoder actually delivers. */
        this.setMap(sheet, v.el.readyState >= 2 ? v.tex : (v.freezeTex || v.baseTex));
    ...
    beats(t) {
      ...
      return {
        ...
        title: band(1.9, 2.7, 5.2, 6.6),
        titleReveal: HARD(clamp01((t - 1.9) / 0.8)),
        endcard: band(26.4, 27.6, 30, 30.1),
        endcardLift: 1 - OUT(clamp01((t - 26.4) / 1.6))
      };
    }
    ...
    async renderFrameAt(t) {
      ...
      for (let pass = 0; pass < 3; pass++) {
        ...
        this.applyFrame(this.t, true);
        if (!this.videos.some(v => v.seekPending || v.freezePending)) break;
      }
      await (this._q || Promise.resolve());
  ...
  function buildTracks() {
    return {
      ...
      reveal: [
        { t: 0.0, x: 0.35, y: -0.15, z: -14.0, ry: 1.25, rz: 0.10, s: 1, o: 0, curl: 0.15, wave: 0.30 },
        { t: 3.2, x: 0.30, y: -0.10, z: -9.5, ry: 1.10, rz: 0.09, s: 1, o: 0.35, curl: 0.20, wave: 0.34, ease: OUT },
