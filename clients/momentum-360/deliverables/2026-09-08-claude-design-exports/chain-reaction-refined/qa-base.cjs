...
const films = fs.readdirSync(path.join(root,'base-source')).filter(f => /^AI 0[1-5] - .*\.html$/.test(f)).sort();
const hash = b => crypto.createHash('sha256').update(b).digest('hex');
const log = o => process.stdout.write(JSON.stringify(o)+'\n');
(async()=>{
 const browser = await chromium.launch({channel:'chrome',headless:true});
 const receipt = {started:new Date().toISOString(),full,sourceArchiveSHA256:'70CA903E4C0E507642FC7B8018D0415FDD8F968BAB0D7AAA04BB82DDCE0292F0',films:[]};
 try {
  for(const filename of films){
   ...
   await page.goto('http://127.0.0.1:57907/'+encodeURIComponent(filename),{waitUntil:'networkidle',timeout:60000});
   ...
   for(const aspect of ['16:9','9:16']){
    ...
    const samples=[];
    const frames=full?Array.from({length:540},(_,i)=>i):[0,42,165,282,330,390,492,539];
    for(const n of frames){
     ...
     if(!full || n%30===0){
      ...
      samples.push({frame:n,width:info.width,height:info.height,clear,soft,dirty,visible});
