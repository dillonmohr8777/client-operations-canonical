...
def slack_text_matches(received, sent):
    # Slack escapes &, < and > on ingest, so exact equality fails on any answer holding a
    # markdown link or angle brackets, marking delivered messages UNCERTAIN. Unescape first;
    # &amp; must be last so &amp;lt; does not collapse two steps into one.
    return received.replace('&lt;', '<').replace('&gt;', '>').replace('&amp;', '&') == sent
...
{VAULT / '12_Brain'} is the knowledge, decision, research, project and memory layer: search it first
and follow its links, starting from {VAULT / 'INDEX.md'} and {VAULT / '12_Brain/00_Home.md'}. Do not
preload the whole vault. Also consult {VAULT / 'System/operating-status.md'} and
{VAULT / 'System/approval-queue.md'} for current state before asserting anything present tense.
...
class Codex:
    ...
    def run(self, prompt, thread_id, stop, on_thread):
        ...
        params = {'cwd': str(ROOT), 'model': 'gpt-5.6-sol', 'approvalPolicy': 'on-request',
        ...
        while time.monotonic() < deadline:
            ...
            if event.get('method') == 'item/completed':
                ...
                if item.get('type') == 'agentMessage':
                    # Codex can emit several agentMessages per turn; keeping only the last
                    # silently dropped the real answer and delivered a trailing footer.
                    part = item.get('text', '')
                    final = (final + chr(10)*2 + part).strip() if final and part else (part or final)
    ...
    def process(self, job):
        ...
        try:
            ...
            with self.lock:
                ...
                if not any(m.get('ts') == ts and m.get('user') == self.bot_user
                           and slack_text_matches(m.get('text', ''), answer)
