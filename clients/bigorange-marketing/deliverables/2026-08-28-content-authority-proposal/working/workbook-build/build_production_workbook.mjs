...
const OUTPUT_FILE = path.join(OUT, "BigOrange-19-Asset-Content-Production-Plan-2026-08-28.xlsx");
...
const ownerRows = rawRows.map((row) => {
  ...
  return [
    ...
    rejected ? "Rejected from builder-marketing plan" : (asset.production_state || asset.scope_state),
...
const cards = [
  ...
  ["J5:K7", "Complete local review drafts", "=COUNTA('Asset Register'!$A$5:$A$23)", "19 owned URLs; publication gates preserved"]
];
...
summary.getRange("A10:D14").values = [
  ...
  ["Current production", "19 complete local drafts", "3 original pilot + 16 local extension", "No staging or publication approval implied"],
...
summary.getRange("H10:H14").values = [["Original pilot scope; local drafts complete"], ["Local drafts complete; publication gates preserved"], ["Guide draft complete; social execution remains excluded"], ["Local drafts complete; sequencing gates preserved"], ["Decision-support draft only until offer or method is validated"]];
    ...
    : a.build_window.includes("next phase") ? "Next phase"
      : a.build_window === "Phase 2" ? "Phase 2"
        : a.build_window.startsWith("Backlog") ? "Backlog"
          : "Validate first";
  return [a.asset_id, a.type, a.title, a.slug, a.reader_job, a.funnel_stage, bucket, a.build_window, a.production_state || a.scope_state, a.cta, a.source_needs, a.internal_links];
});
