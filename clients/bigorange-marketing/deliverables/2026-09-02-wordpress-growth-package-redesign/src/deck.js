...
{ const s=pres.addSlide(); s.background={color:INK};
  ...
  T(s,'Where your WordPress site stands.',{x:0.6,y:2.4,w:8.2,h:1.9,fontFace:H,fontSize:50,bold:true,color:W,charSpacing:-2});
  T(s,'A 370 URL technical plan, a 24 asset authority system, five new builder blogs, and the checks that turn content into real leads.',{x:0.6,y:4.45,w:7.6,h:1.0,fontFace:B,fontSize:15,color:'EDE7E0'});
  ...
  notes(s,'Start with the headline: this is a controlled system, not a dump-everything content push.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
// ---------- 2 Original agreement ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,2); head(s,'Original agreement','What we actually agreed to');
  const cols=[['Milestone 1','15 hours','A full audit, a keyword and content strategy, plus the architecture, priorities and plan for building it.'],
              ['Milestone 2','20 hours','One pillar page, two supporting articles, and hands-on technical SEO and WordPress help.'],
              ['Commercial frame','$1,050 total','35 hours at $30 an hour. Phase 2 social, downloadable and webinar work stayed out of scope for this pilot.']];
  ...
  notes(s,'Ground us in the signed scope first, then get into everything on top of it.\n\n[Sources]\nOriginal BigOrange proposal and working packet.\n[/Sources]');
}
// ---------- 3 Expanded investment ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,3); head(s,'Expanded investment','What is ready for you to look at');
  ...
  card(s,7.6,2.75,5.13,3.85,'No-charge proof of commitment','More than the paid pilot','The five new blogs, a deeper page-level optimization framework, six branded PDFs, implementation packaging and this leadership deck are all extra, my way of showing how invested I am in BigOrange and the work you do for clients.\n\nPublic release, imagery and factual claims still need BigOrange approval.',{dark:true,bodySize:12});
  T(s,'Extra work, done at no charge, on top of the signed 35 hour pilot.',{x:0.6,y:5.0,w:6.6,h:1.2,fontFace:H,fontSize:18,bold:true,color:INK});
  notes(s,'Frame the no-charge work as a statement of investment, not an open-ended scope.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
// ---------- 4 Current state ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,4); head(s,'Current state','One site, working start to finish');
  ...
  card(s,7.6,2.75,5.13,3.85,'Private review boundary','Nothing new is public yet','The new blogs and collateral are sitting in private review. What’s live now stays live until a named owner signs off on the facts, images, links, and when it goes out.',{bodySize:12.5});
  const ic=await icon(I.FiLock,O); s.addImage({data:ic,x:0.6,y:5.1,w:0.5,h:0.5});
  T(s,'The builder hub you have live today isn’t touched.',{x:1.25,y:5.12,w:6,h:0.5,fontFace:H,fontSize:14,bold:true,color:INK,valign:'middle'});
  notes(s,'Be plain about the line here: what’s live stays live until someone signs off.\n\n[Sources]\nSeptember 1, 2026 public crawl and WordPress inventory.\n[/Sources]');
}
// ---------- 5 Evidence baseline ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,5); head(s,'Evidence baseline','Where the site is losing ground');
  const st=[['8','P0 priorities'],['38','P1 priorities'],['21','pages missing from sitemap'],['15','published nofollow pages']];
  st.forEach((v,i)=>stat(s,0.6+i*1.72,2.75,1.6,2.0,v[0],v[1],i===0,36));
  card(s,7.6,2.75,5.13,3.85,'Highest-impact fixes','Fix the signals before you scale content','Fix the homepage “Stategic” typo across meta, Open Graph and schema. Cut the AI services page from four H1s to one. Check nofollow page by page. Fix sitemap eligibility. Re-test the builder hub on mobile after the media and script work.',{dark:true,bodySize:12});
  const ic=await icon(I.FiAlertTriangle,O); s.addImage({data:ic,x:0.6,y:5.1,w:0.5,h:0.5});
  T(s,'These change how the site gets read, found, and shown.',{x:1.25,y:5.12,w:6.1,h:0.5,fontFace:H,fontSize:14,bold:true,color:INK,valign:'middle'});
  notes(s,'These fixes matter because they change how the site gets read, found, and shown.\n\n[Sources]\nSeptember 1, 2026 public crawl.\nAugust 20, 2026 builder hub mobile performance sample.\n[/Sources]');
}
// ---------- 6 Technical priority (dark) ----------
{ const s=pres.addSlide(); s.background={color:INK}; chrome(s,6,true); head(s,'Technical priority','The builder hub is slow, and that matters',true);
  ...
  T(s,'We’ll re-test after fixing image sizing, formats, preload, fonts, third-party scripts and critical CSS. One sample is a starting point, not a final grade.',{x:8.55,y:3.55,w:4.2,h:2.5,fontFace:B,fontSize:14,color:'EDE7E0'});
  notes(s,'Stay disciplined here: it’s one sample, and we remeasure after the fixes land.\n\n[Sources]\nAugust 20, 2026 builder hub mobile performance sample.\n[/Sources]');
}
// ---------- 7 Keyword truth ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,7); head(s,'Keyword truth','Protect what ranks. Build where the gaps are real.');
  ...
  card(s,7.6,2.75,5.13,1.85,'Builder lane','17 of 21 terms rank','Thirteen builder terms already sit in positions 1–3, all pointing to the builder hub. That page stays the one that sells.',{titleSize:15,bodySize:10.5});
  ...
  notes(s,'Remember Moz is a snapshot in time, not live rankings.\n\n[Sources]\nMoz snapshot dated August 14, 2026.\n[/Sources]');
}
// ---------- 8 Page contract ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,8); head(s,'Every indexable page','One page, ten checks, one owner');
  ...
  notes(s,'This is the release checklist for every page. Wording and proof still differ by template.\n\n[Sources]\nGoogle Search Central: creating helpful, reliable, people-first content.\nGoogle Search Central: Article structured data.\n[/Sources]');
}
// ---------- 9 Authority model ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,9); head(s,'Authority model','24 assets, working like one system');
  ...
  T(s,'One page owns each topic, so pages stop competing with each other. Links guide readers to the right answer, then the next step.',{x:0.6,y:5.7,w:11,h:0.9,fontFace:H,fontSize:16,bold:true,color:INK});
  notes(s,'Explain that the service page stays the sales destination, and the articles just answer the narrower questions.\n\n[Sources]\nBigOrange authority package inventory and internal-link map.\n[/Sources]');
}
// ---------- 10 Five blogs ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,10); head(s,'Five separate blogs','Each owns one verified gap');
  ...
  notes(s,'Each of these five stands alone, not a chapter of one bigger article.\n\n[Sources]\nFive-blog package manifest dated September 2, 2026.\n[/Sources]');
}
// ---------- 11 Answer-engine readiness ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,11); head(s,'Answer-engine readiness','What’s built into every blog');
  const Q=[['Answer first','Fast comprehension','A direct answer, a clear definition and descriptive headings make the page easy to read, for people and machines.',I.FiZap],
           ['Decision support','Information gain','Tables, steps, tradeoffs and real review questions make the page more useful than a generic summary.',I.FiGrid],
           ['Trust','Proof boundaries','Current sources, clear caveats, named review gates, and never an invented result or guarantee.',I.FiShield],
           ['Machine context','Structured data','BlogPosting and Breadcrumb data mirror what’s on the page. FAQs are written for readers, not for a rich-result promise.',I.FiCode]];
  ...
  notes(s,'Usefulness on the page comes first. Schema just helps machines read it, it doesn’t manufacture authority.\n\n[Sources]\nGoogle Search Central: creating helpful, reliable, people-first content.\nGoogle Search Central: Article structured data.\n[/Sources]');
}
...
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,12); head(s,'WordPress backend','Draft → review → release → verify');
  ...
  T(s,'No draft hits the sitemap. Nothing goes public without a named release owner.',{x:1.25,y:5.78,w:10.5,h:0.65,fontFace:H,fontSize:15,bold:true,color:INK,valign:'middle'});
  notes(s,'This is the private staging gate in WordPress. Drafts stay reversible, and approval is explicit.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
// ---------- 13 Measurement chain ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,13); head(s,'Measurement','Rankings only matter once they lead to a sale');
  ...
  notes(s,'Don’t let a rank or a view stand in for revenue. Walk the whole chain.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
// ---------- 14 90-day rollout ----------
{ const s=pres.addSlide(); s.background={color:W}; chrome(s,14); head(s,'90-day rollout','Fix it first. Ship in waves. Learn before scaling.');
  ...
  T(s,'12-month scale: we only add more where Search Console, client questions and sales feedback say it’s worth it.',{x:0.6,y:5.8,w:11.8,h:0.7,fontFace:H,fontSize:14,bold:true,color:INK,valign:'middle'});
  notes(s,'We’re pacing this on purpose, so the team can learn and protect what already ranks.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
// ---------- 15 Thursday close (dark) ----------
{ const s=pres.addSlide(); s.background={color:INK}; chrome(s,15,true); head(s,'Thursday close','Seven decisions that get this moving',true);
  ...
  T(s,'The goal is a named owner, a release order and a way to measure it, not a longer idea list.',{x:0.6,y:5.85,w:11.8,h:0.7,fontFace:B,fontSize:14,color:'EDE7E0',valign:'middle'});
  notes(s,'Close by locking in owners and the first release dates. Keep Phase 2 as its own decision.\n\n[Sources]\nInternal BigOrange working package; September 2, 2026.\n[/Sources]');
}
