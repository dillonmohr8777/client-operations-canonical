...
def render_social_graphic(
    ...
    row_y = panel_top + 38
    accent_colors = [TEAL, GOLD, CORAL, BLUE]
...
def draw_cover(pdf: canvas.Canvas, month: dict) -> None:
    ...
    pdf.setFillColor(colors.HexColor(TEAL))
...
def run_qa(data: dict, graphics: list[Path], pdfs: list[Path]) -> dict:
    ...
    if len(data["months"]) != 2:
        errors.append(f"Expected two monthly packets, found {len(data['months'])}.")
    if len(pieces) != 8:
        errors.append(f"Expected eight separate content pieces, found {len(pieces)}.")
    if len(graphics) != 8:
        errors.append(f"Expected eight separate graphics, found {len(graphics)}.")
    if len(pdfs) != 2:
        errors.append(f"Expected two separate monthly PDFs, found {len(pdfs)}.")

    for graphic in graphics:
        with Image.open(graphic) as image:
            if image.size != SOCIAL_SIZE:
                errors.append(f"Incorrect graphic size: {graphic.name} {image.size}.")
    ...
    for month, pdf_path in zip(data["months"], pdfs):
        ...
        if document.page_count != 5:
            errors.append(f"{pdf_path.name} has {document.page_count} pages; expected 5.")
        for page in document:
    ...
    return {
        ...
        "checks": {
            "two_separate_monthly_pdfs": len(pdfs) == 2,
            "eight_separate_content_graphics": len(graphics) == 8,
