...
Current local and open-weight recommendations live in `registry/local-models.json`. The Cursor picker pack is `registry/cursor-local-models.json`. The roster is prepare-only evidence: it does not pull Ollama weights or change OmniRoute defaults. DeepSeek-V4-Pro-0813 is the current DeepSeek Pro record. Validate with `scripts/Test-LocalModelRoster.ps1` and `scripts/Test-CursorLocalModels.ps1`. Institute the desktop plugin with `scripts/Install-CursorLocalModels.ps1 -Apply`. Inspect live loopback health with `scripts/Update-AiStackState.ps1 -NoWrite`. See `docs/LOCAL_MODELS.md`.
