...
Current local and open-weight recommendations live in `registry/local-models.json`. The Cursor picker pack is `registry/cursor-local-models.json`. The roster is prepare-only evidence: Cloud Agents must not pull Ollama weights or open a tunnel. Pull the 24 GB or 16 GB daily set on DESKTOP or AHCM, confirm tags are local rather than `:cloud`, then point desktop Cursor at `http://127.0.0.1:11434/v1`. DeepSeek-V4-Pro-0813 is the current DeepSeek Pro record and is not an Ollama local pull. Validate with `scripts/Test-LocalModelRoster.ps1` and `scripts/Test-CursorLocalModels.ps1`. Institute the desktop plugin with `scripts/Install-CursorLocalModels.ps1 -Apply`. After a desktop pull, inspect live loopback health with `scripts/Update-AiStackState.ps1 -NoWrite`. See `docs/LOCAL_MODELS.md`.
