...
function EarthZoomShot() {
  ...
  if (!artist.heroVideo) return null

  return (
    <span className="pop-box sheen earth-zoom-shot duality-shot block">
      <img
        src={artist.heroPoster ?? artist.heroImage ?? ''}
        alt={`${artist.name} arriving from orbit`}
        className="earth-zoom-poster block w-full object-cover"
        width={1280}
        height={720}
        fetchPriority="high"
        decoding="async"
      ...
      <video
        ref={videoRef}
        className="earth-zoom-video"
        autoPlay
        muted
        loop
        playsInline
        preload="auto"
        poster={artist.heroPoster ?? undefined}
        aria-hidden="true"
      >
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-5xl flex-col items-center">
        ...
        {/* Earth Zoom of Dillon leads: orbit to street, then the 814 shot */}
        <div className="reveal reveal-late relative mt-8 w-full max-w-4xl">
