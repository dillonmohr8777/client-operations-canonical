[CmdletBinding()]
param([string]$TaskName = 'MarketingChief-BuzzAgentWorker')
...
if (-not (Test-Path -LiteralPath $wrapper -PathType Leaf) -or -not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw 'The console-free scheduled-task wrapper is unavailable.'
}
$manifestEntry = Get-Content -LiteralPath $manifestPath | Where-Object { $_ -like "$TaskName`t*" } | Select-Object -First 1
if (-not $manifestEntry) { throw "The hidden-task manifest does not contain $TaskName." }
...
if ($LASTEXITCODE -ne 0) { throw "Could not register the hidden Buzz Codex worker task: $($result -join ' ')" }
