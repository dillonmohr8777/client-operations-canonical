...
try{
    ...
    $systemQueue=Copy-JsonObject $queue;$systemQueue.revision=601;$systemQueue.updatedAt='2026-07-16T12:45:00Z'
    ...
    $systemUpdate=Invoke-Script (Join-Path $PSScriptRoot 'Update-MarketingSystemState.ps1') $systemArgs;Add-ExitCheck 'system-state:isolated-update' $systemUpdate;$systemUpdateJson=Read-JsonOutput $systemUpdate;$updatedSystemQueue=Get-Content -LiteralPath $systemQueuePath -Raw -Encoding UTF8|ConvertFrom-Json;$updatedSystemItem=@($updatedSystemQueue.systemBacklog|Where-Object{$_.id-eq'sys-20260716-0004'})[0];$clientVersionsAfter=@($updatedSystemQueue.workItems|ForEach-Object{'{0}:{1}'-f$_.id,[int]$_.version})-join'|'
    ...
    $systemMutation=if(Test-Path -LiteralPath $systemMutationLog){Get-Content -LiteralPath $systemMutationLog -Tail 1|ConvertFrom-Json}else{$null};Add-Check 'system-state:audit-recorded' ($null-ne$systemMutation-and$systemMutation.action-eq'system-state-update'-and$systemMutation.systemBacklogId-eq'sys-20260716-0004'-and[int]$systemMutation.priorQueueRevision-eq601-and[int]$systemMutation.queueRevision-eq602-and-not[bool]$systemMutation.externalActionTaken) ([string]$systemMutation.action)
    ...
    $coverageTemp=Join-Path $tempRoot 'access-registry.json';$coverageDoc=[ordered]@{clients=@([ordered]@{id='fagan-painting';systems=@([ordered]@{last_verified_at='2026-07-16T06:00:00Z';secret_ref='bw://item/00000000-0000-4000-8000-000000000001'},[ordered]@{last_verified_at='2025-01-01T00:00:00Z';secret_ref='bw://item/00000000-0000-4000-8000-000000000002'},[ordered]@{last_verified_at='not-a-date';secret_ref='bw://item/00000000-0000-4000-8000-000000000003'},[ordered]@{last_verified_at='2026-07-17T00:00:00Z';secret_ref='bw://item/00000000-0000-4000-8000-000000000004'})},[ordered]@{id='dillon-operations';systems=@([ordered]@{id='bitwarden-password-manager';account_role='primary-codex-and-claude-client-vault';chrome_extension_state='installed-and-vault-open-in-persistent-chrome-account-identity-unverified'})})};Write-Utf8Json $coverageTemp $coverageDoc
    $coverageStateTemp=Join-Path $tempRoot 'access-coverage.json';$coverageResult=Invoke-Script (Join-Path $PSScriptRoot 'Update-AccessCoverage.ps1') @('-OutputPath',$coverageStateTemp,'-AccessRegistryPath',$coverageTemp,'-AsOf','2026-07-16T07:00:00Z','-VerificationFreshnessDays','30');Add-ExitCheck 'access-coverage:timestamp-validation' $coverageResult;$coverage=Read-JsonOutput $coverageResult;Add-Check 'access-coverage:fresh-stale-invalid-separated' ([int]$coverage.summary.verifiedSystemCount-eq1-and[int]$coverage.summary.staleVerificationCount-eq1-and[int]$coverage.summary.invalidVerificationCount-eq2) ("fresh={0};stale={1};invalid={2}"-f$coverage.summary.verifiedSystemCount,$coverage.summary.staleVerificationCount,$coverage.summary.invalidVerificationCount)
    ...
    $healthCompleted=Invoke-Script (Join-Path $PSScriptRoot 'Update-MarketingSystemHealth.ps1') @('-NoWrite','-CredentialBridgePath',$fakeBridge,'-AccessBrokerScriptPath',$completedAccess,'-AccessCoveragePath',$coverageStateTemp,'-ExternalProbeTimeoutSeconds','5');Add-ExitCheck 'health:completed-external-probe' $healthCompleted;$healthCompletedJson=Read-JsonOutput $healthCompleted;Add-Check 'health:completed-probe-exit-preserved' ([bool]$healthCompletedJson.accessBroker.valid-and-not[bool]$healthCompletedJson.accessBroker.timedOut) 'valid-non-timeout-exit-zero'
    $identityGateSet=@($healthCompletedJson.humanGates);Add-Check 'health:identity-confirmation-replaces-unlock' ('bitwarden_primary_vault_identity_confirmation'-in$identityGateSet-and'bitwarden_chrome_extension_unlock'-notin$identityGateSet-and'exact_bw_item_locator_mapping'-in$identityGateSet-and'secondary_exposed_password_rotation'-in$identityGateSet) ($identityGateSet-join',')
    $fallbackCoveragePath=Join-Path $tempRoot 'access-coverage-fallback.json';Write-Utf8Json $fallbackCoveragePath ([ordered]@{schemaVersion=1;summary=[ordered]@{};priority=@()});$healthFallback=Invoke-Script (Join-Path $PSScriptRoot 'Update-MarketingSystemHealth.ps1') @('-NoWrite','-CredentialBridgePath',$fakeBridge,'-AccessBrokerScriptPath',$completedAccess,'-AccessCoveragePath',$fallbackCoveragePath,'-ExternalProbeTimeoutSeconds','5');Add-ExitCheck 'health:unknown-vault-state-probe' $healthFallback;$healthFallbackJson=Read-JsonOutput $healthFallback;Add-Check 'health:unknown-vault-state-fails-closed' ('bitwarden_chrome_extension_unlock'-in@($healthFallbackJson.humanGates)) (@($healthFallbackJson.humanGates)-join',')
    $verifiedCoveragePath=Join-Path $tempRoot 'access-coverage-verified.json';Write-Utf8Json $verifiedCoveragePath ([ordered]@{schemaVersion=2;summary=[ordered]@{};priority=@();credentialVault=[ordered]@{primaryVaultIdentityStatus='verified'}});$healthVerified=Invoke-Script (Join-Path $PSScriptRoot 'Update-MarketingSystemHealth.ps1') @('-NoWrite','-CredentialBridgePath',$fakeBridge,'-AccessBrokerScriptPath',$completedAccess,'-AccessCoveragePath',$verifiedCoveragePath,'-ExternalProbeTimeoutSeconds','5');Add-ExitCheck 'health:verified-vault-state-probe' $healthVerified;$healthVerifiedJson=Read-JsonOutput $healthVerified;$verifiedGateSet=@($healthVerifiedJson.humanGates);Add-Check 'health:verified-vault-removes-identity-gate' ('bitwarden_chrome_extension_unlock'-notin$verifiedGateSet-and'bitwarden_primary_vault_identity_confirmation'-notin$verifiedGateSet-and'exact_bw_item_locator_mapping'-in$verifiedGateSet-and'secondary_exposed_password_rotation'-in$verifiedGateSet) ($verifiedGateSet-join',')
