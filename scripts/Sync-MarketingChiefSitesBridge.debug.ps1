...
function Invoke-MachineGet {
    ...
    Write-Output ("DBG GET TokenLen=" + ([string]$Token.Length) + " DispatchLen=" + ([string]$DispatchToken.Length))
...
function Invoke-MachinePost {
    ...
    Write-Output ("DBG POST TokenLen=" + ([string]$Token.Length) + " DispatchLen=" + ([string]$DispatchToken.Length))
...
try {
    ...
    $studioHeaders = @{'oai-authenticated-user-email' = 'dillonmohr8777@gmail.com';'oai-authenticated-user-full-name' = 'Dillon%20Mohr';'oai-authenticated-user-full-name-encoding' = 'percent-encoded-utf-8'}
    $snapshot = Invoke-RestMethod -Uri ([string]$config.localStudioUrl) -Method Get -Headers $studioHeaders -TimeoutSec 60 -UseBasicParsing
    if (
