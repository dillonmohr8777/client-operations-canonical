...
- `registry/local-models.json` is the recommended local and open-weight model roster. It is prepare-only. Do not treat an Ollama `:cloud` alias as a local install, do not pull weights from the roster, and do not change OmniRoute defaults from it. DeepSeek Pro means `deepseek-ai/DeepSeek-V4-Pro-0813`. Validate with `scripts/Test-LocalModelRoster.ps1`. Live loopback comparison is `scripts/Update-AiStackState.ps1`.
