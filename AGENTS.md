...
> Merged 2026-09-17. The orientation section below came from `CLAUDE.md` in this
> directory. Muse ignores `CLAUDE.md` where an `AGENTS.md` exists, so that
> content was invisible to every Muse run. `CLAUDE.md` now points here instead of
> carrying its own copy. One source of truth, read by every harness.

## Orientation — the canonical client system
...
1. **`slackChannels` is populated on only 6 of 26 routes.** `nkcdc`, `bridge-software`,
   ...
   **Every reconciliation that joins on Slack channel silently fails for those 20.** This is the
   ...
   Note: `System/daily-orchestrator.md` in the vault records this as "5 of 24". The counts
   disagree; this file enumerates the actual routes, so reconcile against `registry\clients.json`
...
- This directory is **chronically occupied** — five sessions logged against it. A Claude Code
  ...
  The lock covers only the working directory, never filesystem access, so give a session a scratch
  folder for its lock and point it at this path.

### Related

- Vault: `C:\Users\dillo\repos\dillon-os` — `01_Clients\` overlays, decisions, brain.
  Its standing brief is `System\daily-orchestrator.md`.
  (Superseded 2026-09-15: the vault was previously on `cursor/immohrtal-standing-canary-3c2e`
  with hundreds of uncommitted files. That branch was merged to main via PR #404 and the reserved
  `NUL` filename that had blocked every commit since 2026-08-23 was removed.)
...
- `/loop` is Dillon's standing Buzz delegation shorthand, not a literal Buzz
  slash command. For each substantive Marketing Chief request, keep Claude's
  chief session as Dillon's only required conversation surface, select the
  smallest relevant set of Buzz agents, message them on Dillon's behalf,
  coordinate their signed replies, and return one reconciled result there. Dillon should
