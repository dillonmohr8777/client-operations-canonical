...
- `registry/local-models.json` is the recommended local and open-weight model roster. `registry/cursor-local-models.json` is the Cursor picker pack. Both are prepare-only. Use Ollama Cloud through Codex desktop `local-ai-worker` tools `ollama_cloud_status` and `ollama_cloud_run`. Do not download or launch new local models or purchase anything. Cursor backend cannot reach desktop loopback, so do not enable Override OpenAI Base URL or localhost BYOK. Cloud Agents keep Cursor-hosted models and must not pull or open a tunnel. Do not treat an Ollama `:cloud` alias as a local install, and do not change OmniRoute defaults from the roster. DeepSeek Pro means `deepseek-ai/DeepSeek-V4-Pro-0813`. Validate with `scripts/Test-LocalModelRoster.ps1` and `scripts/Test-CursorLocalModels.ps1`. Do not run `scripts/Install-CursorLocalModels.ps1 -Apply` from a Cloud Agent. Do not write Cursor API keys, edit `state.vscdb`, or enable tunnels from the roster. Live loopback comparison remains `scripts/Update-AiStackState.ps1 -NoWrite`.
